...
/**
 * How a change is described in words, letters, and theme color ids.
 *
 * These map the Git extension's numeric status enum onto the same badges and
 * colors the built-in Changes list uses, so a row here reads identically to a
 * row there. Theme *ids* are returned rather than `vscode.ThemeColor` objects to
 * keep this module free of VS Code and therefore testable.
 */
